...
//       <div className="relative w-full  bg-[#232428] sm:rounded-xl overflow-hidden shadow-2xl h-full sm:min-h-0">
//         {/* Close button */}
//         <button
...
function EditableName({ value, onChange, placeholder }) {
  ...
  return (
    <button
      ...
      className="group flex items-center gap-1.5 text-left"
    >
      <h2 className="text-white text-xl sm:text-2xl font-bold leading-tight break-all">
        {value}
      </h2>
      <Pencil size={12} className="opacity-0 group-hover:opacity-60 transition-opacity shrink-0" />
    </button>
...
function WidgetCard({ widget, onClick, filled }) {
  return (
    ...
    >
      <img src={widget.img} alt="" className="absolute inset-0 w-full h-full object-cover" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-black/45 group-hover:from-black/90 transition-colors" />

      {/* decorative skeleton lines, top-left, mimicking the loaded-stat header on real Discord widgets */}
      <div className="absolute top-2 left-2 flex flex-col gap-1">
        <span className="block h-1.5 w-10 rounded-full bg-white/25" />
        <span className="block h-1.5 w-7 rounded-full bg-white/15" />
      </div>
      ...
      <div className="absolute bottom-1.5 left-1.5 right-1.5 sm:bottom-2 sm:left-2 sm:right-2 flex items-center gap-1.5">
        {filled && (
          <span className="bg-black/60 rounded-full p-1 shrink-0">
            <Link2 size={10} className="text-white" />
          </span>
        )}
        <span className="text-white text-xs sm:text-sm font-semibold drop-shadow truncate">
...
export default function ProfilePage() {
  ...
  const [name, setName] = useState(userinfo?.name || "Zaid");
  const [username, setUsername] = useState(userinfo?.username || "zaid_1409");
  const [avatar, setAvatar] = useState(userinfo?.profileimg || DEFAULT_AVATAR);
  const [banner, setBanner] = useState(null); // null => illustrated fallback

  const [pronouns, setPronouns] = useState("");
  const [bio, setBio] = useState("");
  const [note, setNote] = useState("");
  const [funFact, setFunFact] = useState("Best dad joke?");
  const [connections, setConnections] = useState([]);
  const [slots, setSlots] = useState([
    { id: "marvel-rivals", label: "Marvel Rivals", img: WIDGET_LIBRARY[0].img, filled: true },
    { id: "wuthering-waves", label: "Wuthering Waves", img: WIDGET_LIBRARY[1].img, filled: true },
  ...
  return (
    <div className="flex items-center justify-center p-0 sm:p-4 font-sans">
      <div className="relative w-full max-w-[95%] bg-[#232428] sm:rounded-xl overflow-hidden shadow-2xl h-full sm:min-h-0">
        {/* Close button */}
        <button
          onClick={() => setOpen(dispatch(closeProfilePageSettings()))}
          className="absolute top-3 right-3 z-20 bg-black/40 hover:bg-black/60 rounded-full p-1.5 transition-colors"
        ...
        <div className="flex flex-col sm:flex-row">
          {/* Left column */}
          <div className="w-full sm:w-[300px] shrink-0 bg-[#232428]">
            {/* Banner — illustrated fallback if none set */}
            <ImagePicker onSelectRaw={(src) => setEditingImage({ shape: "banner", src })}>
              <div className="relative h-[110px] sm:h-[130px] w-full group overflow-hidden">
                {banner ? (
                  <img src={banner} alt="banner" className="absolute inset-0 w-full h-full object-cover" />
                ) : (
                  <div className="absolute inset-0 bg-gradient-to-br from-[#464646] via-[#353535] to-[#0f0f0f] overflow-hidden">
                    {/* <span className="absolute top-3 right-10 w-9 h-9 rounded-full bg-white shadow-[0_0_25px_8px_rgba(255,255,255,0.5)]" />
                    <span className="absolute -bottom-6 -left-6 w-32 h-20 rounded-full bg-[#33507e]/60 blur-sm" />
                    <span className="absolute bottom-0 left-10 w-20 h-12 rounded-full bg-[#2c4468]/70 blur-[2px]" />
                    <span className="absolute -bottom-3 right-0 w-28 h-16 rounded-full bg-[#1f3252]/70 blur-[2px]" />
                    <span className="absolute bottom-2 left-1/3 w-2 h-2 rounded-full bg-[#1f3252]" /> */}
                  </div>
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-[#232428] to-transparent" />
                <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/30 transition-colors">
            ...
            <div className="px-4">
              <div className="relative -mt-10 mb-2 w-fit">
                <ImagePicker onSelectRaw={(src) => setEditingImage({ shape: "avatar", src })}>
                  <div className="relative w-20 h-20 group">
                    <img
                      src={avatar}
                      alt="avatar"
                      className="w-20 h-20 rounded-full border-[5px] border-[#232428] object-cover bg-[#1e1f22]"
                    />
                    <div className="absolute inset-[5px] rounded-full flex items-center justify-center bg-black/0 group-hover:bg-black/40 transition-colors">
                      <Camera
                        size={16}
                ...
                {/* fun-fact bubble, floats to the right of the avatar */}
                <EditableText
                  value={funFact}
                  onChange={setFunFact}
                  placeholder="Add a fun fact"
                  className="absolute left-[74px] -top-1 w-max max-w-[150px] !inline-flex bg-[#2b2d31]/95 hover:bg-[#35373c] border border-white/10 rounded-full pl-2.5 pr-2 py-1 text-white text-[11px] font-medium shadow-lg whitespace-nowrap"
                />
              </div>

              <EditableName value={name} onChange={setName} placeholder="Your name" />

              <div className="flex items-center gap-1.5 text-[#949ba4] text-xs mt-1 mb-3 flex-wrap">
                {editingUsername ? (
                  <div className="flex items-center gap-1">
                    <input
                      autoFocus
                ...
                <EditableText
                  ...
                  className="text-xs text-[#949ba4] hover:text-white w-auto"
                />
                <button className="flex items-center gap-1 bg-[#2b2d31] hover:bg-[#35373c] rounded-full pl-2 pr-1.5 py-0.5 text-[11px] font-semibold text-[#dbdee1] transition-colors">
                  <span className="w-3.5 h-3.5 rounded-full bg-gradient-to-br from-[#5865f2] to-[#eb459e]" />
                  CODE
                  <ChevronDown size={11} />
                </button>
                <span className="w-4 h-4 rounded-full bg-gradient-to-br from-[#00c2ff] to-[#5865f2]" />
                <span className="w-4 h-4 rounded-full bg-gradient-to-br from-[#f23f5f] to-[#eb459e]" />
              </div>

              <div className="flex items-center gap-2 mb-4">
                <button className="flex-1 flex items-center justify-center gap-1.5 bg-[#5865f2] hover:bg-[#4752c4] text-white text-sm font-medium py-2 rounded-md transition-colors">
              ...
              <div className="mb-4">
                ...
                <p className="text-[#dbdee1] text-sm">May 9, 2025</p>
              </div>
