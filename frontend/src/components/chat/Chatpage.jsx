...
import { Phone,ChevronLeft  , Video, Pin, Users, Gift, Smile, Grid3x3, Image as ImageIcon, Plus } from 'lucide-react';
...
const MESSAGES = [
  {
    ...
    time: '7:14 AM',
    type: 'link',
    content: 'https://www.instagram.com/reel/DbhPl79IVQo/?igsh=MW05aHhtNGptb2pheA==',
  },
  {
    id: 'm2',
    author: 'Sawan Kumar',
...
const AVATAR_PALETTE = ['#5865f2', '#3ba55d', '#c07a3e', '#eb459e', '#faa61a', '#ed4245', '#9b59b6'];
    ...
    >
      ...
      <div className="min-w-0">
        <div className="flex items-baseline">
          ...
          <span className="text-[11px] text-[#949ba4] ml-2">{message.time}</span>
        </div>
        ...
        {message.type === 'text' && (
          <p className="text-[15px] text-[#dbdee1] leading-[1.375rem]">{message.content}</p>
        )}

        {isEmojiOnly && <p className="text-[28px] leading-[1.6] -mt-0.5">{message.content}</p>}
      </div>
    </div>
  );
}
...
export default function ChatPage({setChatopen}) {
  ...
  useEffect(() => {
    ...
    const timers = MESSAGES.map((_, i) =>
      setTimeout(() => setVisibleCount((c) => Math.max(c, i + 1)), 120 + i * 90)
    );
    return () => {
  ...
  return (
    <div className="flex-1 min-w-0 min-h-0 h-full bg-[#0000008e] flex flex-col">
      ...
      <div
        ...
        style={{ opacity: headerIn ? 1 : 0, transform: headerIn ? 'translateY(0px)' : 'translateY(-6px)' }}
      >
        
        <div className="flex items-center gap-2.5 min-w-0">
            <button ><ChevronLeft   onClick={()=>{setChatopen(false)}} className='text-white' /></button>
          <Avatar name={CONTACT.name} size={30} />
          <span className="font-semibold text-[15px] text-white truncate">{CONTACT.name}</span>
        </div>
      ...
      <div className="px-6 pt-3 pb-2 shrink-0">
        <p className="text-[13px] text-[#5c5e66] truncate select-none">
          Build with Visual Studio Code, anywhere, anytime, already in your browser.
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-2">
        <div className="flex items-center gap-3 px-6 my-3 select-none">
          ...
          <span className="text-[12px] text-[#949ba4] font-medium">August 3, 2026</span>
          <div className="h-px bg-[#3f4147] flex-1" />
        </div>

        <div className="flex flex-col gap-2.5">
          {MESSAGES.map((message, i) => (
            <MessageRow key={message.id} message={message} index={i} visible={i < visibleCount} />
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pb-6 p-8 pt-1 shrink-0">
        <div className="flex items-center gap-3 bg-[#383a40] rounded-lg px-4 py-2.5 focus-within:ring-1 focus-within:ring-[#4a4d55] transition-all duration-200">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <InputIcon label="Open emoji picker"><Smile size={20} /></InputIcon>
          </div>
        </div>
      </div>
    </div>
