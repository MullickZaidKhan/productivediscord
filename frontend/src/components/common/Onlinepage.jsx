...
useEffect(() => {
  ...
  return (
    <div>
      ...
      >
        {isLoading ? (
          <div className="px-4 py-6 text-sm text-[#949ba4]">Loading friends...</div>
        ) : isError ? (
          <div className="px-4 py-6 text-sm text-[#f87171]">Failed to load friends.</div>
        ) : onlineFriends.length === 0 ? (
          <div className="px-4 py-6 text-sm text-[#949ba4]">
            {isConnected ? "No friends online right now." : "Connecting..."}
          </div>
        ) : (
